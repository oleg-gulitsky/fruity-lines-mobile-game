...
namespace GameLogic
{
  public static class Pathfinder
  {
    ...
    public static List<CellData> GetPath(FruitType[,] cells, CellData currentPosition, CellData targetPosition)
    {
      
      int[,] wave = GetWave(cells, currentPosition, targetPosition);
      List<CellData> path = new();

      if (wave[targetPosition.X, targetPosition.Y] < 1)
        return path;

      CellData current = targetPosition;
    ...
    private static int[,] GetWave(FruitType[,] cells, CellData from, CellData to)
    {
